...
// A short-lived experiment briefly stored products with `images`/`stockQty`
// fields instead of `image`/`stock`. Reads either shape so nothing looks
// broken for records saved during that window; every save from now on
// writes the plain shape again.
function normalizeProduct(id, raw) {
  const image = raw.image || (Array.isArray(raw.images) && raw.images[0]) || '';
  const stock = typeof raw.stock === 'boolean'
    ? raw.stock
    : (typeof raw.stockQty === 'number' ? raw.stockQty > 0 : false);
  return { id, name: raw.name, description: raw.description, price: raw.price, image, stock };
}
